...
1.  `yarn build`
2.  copy the content from the build folder to any webserver.

# Development

1.  run `yarn` to donwload all dependencies.
2.  run `yarn start` to run the development build.
3.  We recommend installing the react and redux-dev-tools and the use of google chrome or chromium during development.
4.  We are using es-lint and prettier.
    * Fix all es lint warnings before commiting anything!
    * Prettier will take care of styling the code.

## Routing

We are using ConnectedRouter which is pretty similiar to the old BrowserRouter from react-router combined with react-router-redux. ConnectedRouter is fully compatible with react-redux.

We are using actions for routing.

Look here for more information: https://github.com/ReactTraining/react-router/tree/master/packages/react-router-redux

Be careful it can be easily confused with the old react-router-redux.
...
### We are using material-ui@next

1.  Documentation can be found here: https://material-ui-next.com/
2.  Check this for examples on Typography: https://material-ui-next.com/style/typography/
3.  Check the Example2 Component for some examples.

#### Icons

1.  We will use the SVG Material Icons as described here: https://material-ui-next.com/style/icons/#svg-material-icons
2.  or create our own Components for other SVG Icons with the SvgIcon Component as described here: https://material-ui-next.com/style/icons/#svg-icons
3.  It is also possible to use the Material Design Icon Font, however we will try to avoid these Icons and may remove the font in the future. http://google.github.io/material-design-icons/#icon-font-for-the-web

## redux-forms

You can use redux-forms to speed up the process of making forms.
